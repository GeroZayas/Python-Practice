...
def check_and_group_files(folder_path):
    file_paths = [os.path.join(folder_path, file) for file in os.listdir(folder_path) if file.endswith(".md")]
    content_hashes = {}
    ...
    for content_hash, similar_files in content_hashes.items():
        if len(similar_files) > 1:
            ...
            for similar_file in similar_files:
                if similar_file != longest_file:
                    destination = os.path.join(new_folder, os.path.basename(similar_file))
