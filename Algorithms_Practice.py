...
def quick_sort(nums:list)->list:
	if len(nums) <=1:
		return nums
	pivot = nums[0] # first element in the array
	left = [] 
	right = [] 
	for num in nums[1:]: # we start with the second element, since the first one is the pivot
		if num < pivot:
			left.append(num)
		else:
			right.append(num)
	return quick_sort(left) + [pivot] + quick_sort(right) # we use recursion and concatenate the lists
...
print("\n\n",binary_search(sorted_numbers, 0, len(sorted_numbers)-1, 4))
